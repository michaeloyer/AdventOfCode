...
let puzzle = """dark olive bags contain 2 muted brown bags, 1 mirrored tomato bag, 4 bright black bags.
...
module Part1 =
    ...
    let colorContentsDictionary =
        run colorContentsDictionaryParser puzzle
        |> function
    ...
    let answer = getPotentialContainers Set.empty ["shiny gold"]
                 |> Set.count
...
dark violet bags contain no other bags."""
    let containerDictionary =
        run containerDictionaryParser puzzle
        |> function
            | Success (dictionary, _, _) -> dictionary
